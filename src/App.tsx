...
function TransactionsAdmin({ state, update }: { state: AppState; update: (next: Partial<AppState>) => void }) {
  ...
  return (
    <div className="page-stack">
      ...
      <div className="support-filters"><div className="filter-field filter-search"><span>Search</span><label><Search size={16} /><input placeholder="Search client..." value={search} onChange={(event) => setSearch(event.target.value)} /></label></div><label className="filter-field"><span>Status</span><select value={status} onChange={(event) => setStatus(event.target.value as TransactionStatus | "All")}><option>All</option>{STATUSES.map((item) => <option key={item}>{item}</option>)}</select></label><label className="filter-field"><span>From date</span><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label><label className="filter-field"><span>To date</span><input type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label></div>
      <div className="table-card">
        <table>
...
function ActivityLogPage({ state }: { state: AppState }) {
  return <div className="page-stack"><div className="page-title-row"><h1>Activity Log</h1><button className="button outline" disabled={state.activityLogs.length === 0} onClick={() => downloadActivityLogCsv(state.activityLogs)}><Download size={16} /> Export CSV</button></div><div className="table-card"><table><thead><tr><th>Action</th><th>Admin</th><th>Details</th><th>Performed At</th></tr></thead><tbody>{state.activityLogs.map((log) => <tr key={log.id}><td>{log.action}</td><td>{log.adminEmail}</td><td>{log.details}</td><td>{dateTime(log.performedAt)}</td></tr>)}</tbody></table></div></div>;
}

function downloadActivityLogCsv(logs: ActivityLog[]) {
  const headers = ["Action", "Admin", "Details", "Performed At"];
  const body = logs.map((log) => [log.action, log.adminEmail, log.details, dateTime(log.performedAt)].map((cell) => `"${String(cell).replace(/"/g, '""')}"`).join(","));
